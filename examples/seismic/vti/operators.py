...
def kernel_tti_2d(model, u, v, space_order, **kwargs):
    ...
    if forward:
        H0 = (1+2*epsilon)*u.dx2 + sqrt(1 + 2*delta)*v.dy2
        Hz = sqrt(1 + 2*delta)*u.dx2 + v.dy2
        return second_order_stencil(model, u, v, H0, Hz, qu, qv)
    else:
        H0 = ((1+2*epsilon)*u + sqrt(1 + 2*delta)*v).dx2
        Hz = (sqrt(1 + 2*delta)*u + v).dy2
...
def kernel_centered_2d(model, u, v, space_order, **kwargs):
    ...
    if forward:
        H0 = (1+2*epsilon)*u.dx2 + 1/(1+epsilon-delta)*v.dy2
        Hz = (1+epsilon-delta)*(1+2*delta)*u.dx2 + v.dy2
        return second_order_stencil(model, u, v, H0, Hz, qu, qv)
    else:
        H0 = ((1+2*epsilon)*u + (1+epsilon-delta)*(1+2*delta)*v).dx2
        Hz = (1/(1+epsilon-delta)*u + v).dy2
        return second_order_stencil(model, u, v, H0, Hz, qu, qv, forward=forward)


def kernel_self_adj_2d(model, u, v, space_order, **kwargs):
    # Forward or backward
    forward = kwargs.get('forward', True)
...
def JacobianAdjOperator(model, geometry, space_order=4,
    ...
    return Operator(eqn + rec_term + [dm_update], subs=model.spacing_map,
                    name='GradientVTI', **kwargs)


kernels = {('centered', 2): kernel_centered_2d, ('self_adj', 2): kernel_self_adj_2d, 
           ('tti', 2): kernel_tti_2d, ('iso', 2): iso_stencil}
